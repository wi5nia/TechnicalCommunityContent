...
This step helps you download the example solution code, and then push it to your own Visual Studio Team Services account.
...
We want to push the application code to your Visual Studio Team Services account in order to use Team Build for the various HOL.
...
0. Locate the file *PartsUnlimitedMRP.zip* from the folder containing this file, open it, and then copy the contents of the zip file into the repository folder you just created from VSTS.
...
0. Launch the deployment
 ...
 > The UniqueDnsName should be a globally unique (in Azure) name (e.g. MyDevOpsDemo20161001) and will be used as part of the names of resources created in Azure.
...
0. When the deployment completes, you should see the following resources in the Azure Portal:
 ...
 > Take note of the information in the output by the script as you will need it later.
...
0. Go to the project’s homepage in VSTS and click on the **Build** hub at the top of the page.
...
0. For the second PowerShell task, name it `Deploy MRP using sftp and SSH through PowerShell`, and fill in the input values with the following:
 ...
 Arguments:      -sshUser $(sshUser) -sshPassword $(sshPassword) -sshTarget $(sshTarget)
 ```
...
0. Create four variables: `sshUser`, `sshPassword`, `sshTarget` & `instrumentationKey`. Fill in the values of the virtual machine that you created previously using the ARM template. `sshTarget` should be the public DNS name of the virtual machine, such as "mylinuxvm.westus.cloudapp.a
1. 
2. 
3. zure.com."
...
0. Ensure you fill again the `sshUser`, `sshPassword`, `sshTarget` & the `intrumentationKey` with production environment infos.
